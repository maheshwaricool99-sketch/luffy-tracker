...
export async function POST(request: Request) {
  ...
  try {
    const session = await createCheckoutSession(viewer);
    ...
    if (wantsJsonResponse(request)) {
      return jsonError(500, "CHECKOUT_UNAVAILABLE", error instanceof Error ? error.message : "Checkout unavailable");
    }
    const url = new URL("/billing", request.url);
    url.searchParams.set("error", error instanceof Error ? error.message : "Checkout unavailable");
    return NextResponse.redirect(url);
  }
}
