...
export const nseProvider: MarketProvider = {
  id: "nse",
  market: "india",
  priority: 1,
  ...
  async healthCheck() {
    ...
    return result.ok ? successResult("nse", { state: "live" }, result.latencyMs) : failureResult("nse", result.errorCode ?? "PROVIDER_UNAVAILABLE", result.errorMessage ?? "health failed", result.latencyMs);
  },
  async fetchPrices(symbols) {
    ...
    try {
      const prices = await Promise.all(symbols.map(async (symbol) => {
        ...
        if (!Number.isFinite(price) || price <= 0) throw new Error("invalid nse response");
        return { symbol, price, timestampMs };
      }));
      return successResult("nse", pointsFromPrices("nse", prices, true, false), Date.now() - started, false);
    } catch (error) {
      const classified = classifyProviderError(error);
      return failureResult("nse", classified.code, classified.message, Date.now() - started);
    }
  },
};
